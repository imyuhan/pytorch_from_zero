...
`examples/` 下按知识点分了 11 个独立小文件,每个都可以直接 `python 03-attention/examples/NN_xxx.py` 跑通,不依赖 `src/`。详细讲解见 3.5 节。
...
## 3.3 核心概念:注意力机制与多头注意力

> 本节是整章的**动机铺垫**。读完再去 3.4 看 `reshape / transpose / matmul` 这些操作,就知道"为什么这么改 shape"。如果只看代码不读这节,会觉得多头的拆合头像是凭空冒出来的 shape 杂技。
...
#### 3.3.3.2 embedding 与 tensor 的关系
...
#### 3.3.3.3 embedding 在 attention 里的层级
...
> **与本章的关系**:你练习 01 手写的多头注意力,输入 `(3, 4, 4)` 的 tensor 就是广义 embedding——已经查出来的向量序列。真实项目里在这之前还会有两步:tokenizer(字符串 → token ID)和 `nn.Embedding`(ID → 向量)。你 03 章是从"已经有 embedding"开始练的,前两步留到后面的章节。

### 3.3.4 Scaled Dot-Product Attention 公式
...
2. **缩放**:除以 `√d_k`。点积方差随 `d_k` 线性增长,数值太大会让 softmax 把概率压到某个 token 上,梯度消失。详细数学动机见 3.4.7
...
### 3.3.6 为什么把 hidden_dim 拆成 num_heads × head_dim
...
### 3.3.7 num_heads / head_dim 的具体含义
...
### 3.4.3 拆多头: `reshape` + `transpose` + `contiguous`

多头注意力要把 `hidden_dim` 拆成 `num_heads` 和 `head_dim`。这一步是**最容易卡住**的地方。具体为什么这么拆,见 3.3.6。
...
**为什么要 transpose?** 每个 head 要独立算 QK^T,所以 heads 维必须**在 seq 前面**,这样矩阵乘法才能按 head 并行。完整 shape 流转见 3.3.8。

**为什么要 contiguous?** transpose 只是改 stride,不动数据,内存布局变成不规则的。后续如果再 `reshape` / `view` 会报错。

### 3.4.4 `expand` vs `repeat`
...
> **多头版本**的 mask 形状是 `(B, 1, 1, S)`,跟这里 `(B, 1, S)` 的单头版本不一样。详见 3.3.8 的 mask 升维说明。

## 3.5 逐个 example 讲解

> 03 章的 `examples/` 目录下有 11 个独立小文件,**每个文件都可以直接 `python 03-attention/examples/NN_xxx.py` 单独跑通**,完全自包含、只依赖 torch。建议按编号顺序跑(01 → 11),前一个文件建立的概念是后一个文件的基础。

每个小节的"对应章节"会指出**该 example 用到了 3.3 / 3.4 哪些小节的概念**,方便交叉阅读。

---

### 3.5.1 `examples/01_embedding_tensor.py` —— 构造 (batch, seq, hidden) 张量

**对应大纲**:第 1 条(重点)
**对应文档**:3.3.3(embedding 概念)、3.4.1(张量标准形状)
...
- `embeddings.shape = (3, 4, 4)` 三个数分别对应 batch / seq / hidden,记牢这个顺序,后面 03 章所有 shape 都按这个读。
- `valid_mask` 的 False 位置就是模型不该关注的 `[PAD]`。后面 3.5.7 / 3.5.10 会用它做 attention mask。

---

### 3.5.2 `examples/02_unsqueeze_squeeze.py` —— 补 batch 维、删多余维

**对应大纲**:第 2 条(重点)
**对应文档**:3.4.2(unsqueeze / squeeze)
...
### 3.5.3 `examples/03_reshape_transpose.py` —— 拆多头的 shape 流转

**对应大纲**:第 3 条(难点)
**对应文档**:3.3.6 / 3.3.7(为什么拆多头)、3.4.3(reshape + transpose + contiguous)
...
多头注意力的"shape 杂技"全在这三步里。完整原理见 3.3.6 / 3.3.7。这里强调两点:
...
### 3.5.4 `examples/04_expand_repeat.py` —— 广播视图 vs 真实复制

**对应大纲**:第 4 条(重点)
**对应文档**:3.4.4(expand vs repeat)
...
### 3.5.5 `examples/05_cat_stack.py` —— 接长已有维 vs 新建维度

**对应大纲**:第 5 条(重点)
**对应文档**:3.4.5(cat vs stack)
...
### 3.5.6 `examples/06_matmul.py` —— 矩阵乘法的 shape 规则

**对应大纲**:第 6 条(重点)
**对应文档**:3.3.4(QK^T)、3.4.6(matmul 规则)
...
注意:这是 raw scores,**还没除以 √d、还没 softmax、还没 mask** —— 它还不是"注意力权重",只是"原始分数"。完整公式见 3.5.7。
...
### 3.5.7 `examples/07_manual_attention.py` —— 手写 scaled dot-product attention

**对应大纲**:第 7 条(难点 / 整章核心)
**对应文档**:3.3.4(SDPA 公式)、3.3.8(mask 升维)、3.4.7(为什么除以 √d)、3.4.8(mask)
...
#### 3.5.7.x 补充:causal mask(下三角 mask)——单向注意力

> **这一小节是为 `exercises/03_sdpa_attention.py` 第 10 行的提示"is_causal=True 等价于下三角 mask"补的**。3.5.7 主线走的是**双向 attention**(每个 query 能看到所有 key),causal mask 把它改造成**单向**(query i 只能看到 key ≤ i),用于 GPT 这类自回归模型。
...
### 3.5.8 `examples/08_sdpa_compare.py` —— 与官方 SDPA 对齐验证
...
把 3.5.7 的手写 SDPA 公式**再写一遍**,但用 PyTorch 官方的 `F.scaled_dot_product_attention` 跑一次,对比二者输出。生产代码优先用官方函数,但你必须能验证手写公式是对的,生产时切换才放心。
...
8.94e-08 是浮点误差,工程上认为完全相等。如果误差超过 1e-6,**说明公式写错了**,需要回头对照 3.5.7 检查。
...
### 3.5.9 `examples/09_multi_head.py` —— 多头拆头 + 合头

**对应大纲**:第 9 条(难点)
**对应文档**:3.3.5(为什么多头)、3.3.6(参数量守恒)、3.3.8(多头 shape 流转)
**覆盖的代码量**:约 120 行,把 3.5.3 的拆头和 3.5.7 的 SDPA 串起来,演示完整多头注意力。
...
参数量守恒(详细数学见 3.3.6),但能学到多种"关注模式"。训练好的 Transformer 里不同 head 学到的东西差异很大 —— 有的看下一个 token,有的看固定位置,有的看远距离指代。

---

### 3.5.10 `examples/10_mask_visualize.py` —— mask 注意力权重矩阵可视化
...
把 3.5.7 算出的注意力权重矩阵**画出来**(以 ASCII 表格的形式),让 mask 的作用肉眼可见 —— 这是验证 mask 是否正确工作的最快方法。
...
### 3.5.11 `examples/11_batch_size.py` —— batch 维变化演示

**对应大纲**:第 6、7 条的拓展(重点)
**对应文档**:3.4.1(张量标准形状)、3.4.6(matmul 规则)
...
- [ ] 能解释注意力机制要解决 RNN/CNN 的什么痛点(见 3.3.1)
- [ ] 能说清 embedding 是什么,以及 embedding 和 tensor 的关系(见 3.3.3)
- [ ] 能说清 Q / K / V 三个矩阵各自的语义和类比(见 3.3.2)
- [ ] 能解释 Scaled Dot-Product Attention 公式中每一步的作用(见 3.3.4)
- [ ] 能解释多头注意力为什么是"白嫖"——参数量几乎不变却多 8 倍视角(见 3.3.6)
- [ ] 能解释 `hidden_dim = num_heads × head_dim` 三者各自代表什么(见 3.3.7)
- [ ] 能解释 `(B, S, H) @ (H, H) → (B, S, H)` 的广播语义(见 3.4.6)
- [ ] 能口述 `reshape → transpose → contiguous` 三步为什么缺一不可(见 3.4.3)
- [ ] 能手写 `scaled dot-product attention` 并和官方 SDPA 对齐到 1e-6(见 3.5.1.8)
- [ ] 能解释多头注意力为什么用 `head_dim = hidden_dim / num_heads`(见 3.3.6)
- [ ] 能解释 mask 形状从 `(B, S)` 怎么一步步升到 `(B, 1, 1, S)`(见 3.3.8)

## 3.9 下一步

进入 [`06-pretrained-models/doc/06-pretrained-models.md`](../../06-pretrained-models/doc/06-pretrained-models.md) 看预训练模型 —— 真实 backbone 都是卷积 + 注意力的混合架构,这章的矩阵流在 ViT / Swin Transformer 里会直接复用。