...
### 3.2.5 `transforms.Compose` —— 预处理流水线
...
### 3.2.6 `random_split` 切 train/val
...
### 3.3.1 `01_dataset_class.py` — Dataset 三件套
...
### 3.3.2 `02_dataloader_basics.py` — TensorDataset + DataLoader
...
### 3.3.3 `03_list_dataset.py` — 列表型 Dataset
...
### 3.3.4 `04_transforms.py` — transforms 流水线
...
### 3.3.5 `05_train_val_split.py` — random_split
...
### 3.3.6 `06_mnist_load.py` — MNIST 实战
...
进入 [`04-attention/doc/04-attention.md`](../../04-attention/doc/04-attention.md) 学维度变换与注意力矩阵 —— 把 01 章的 QKV shape 流补成完整的 scaled dot-product attention 和多头。
